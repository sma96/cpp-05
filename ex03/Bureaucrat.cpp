...
void                Bureaucrat::signForm(Form &F)
{
    ...
    {
        F.beSigned(*this);
        std::cout << C_GREN << getName() << " signs " << F.getName() << C_NRML << std::endl;
    }
    catch (std::exception &e)
    {
        std::cout << C_RED <<getName() << " can't sign " << F.getName() << " " << e.what() << C_NRML << std::endl;
    }
}
...
void                Bureaucrat::executeForm(Form const &form) const
{
    try{
        form.execute(*this);
        std::cout << C_GREN << name << " executes " << form.getName() << C_NRML << std::endl;
    }
    catch (std::exception &e)
    {
        std::cout<< C_RED << name << " can't execute " << form.getName()  << " : " <<  e.what() << C_NRML << std::endl;
    }
}
